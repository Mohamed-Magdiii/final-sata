...
export const addShipping = (formData)=>async dispatch =>{
    try {
        const res = await Axios.post(`${process.env.REACT_APP_API_URL}/api/region` , formData , {headers:{'x-auth-token':localStorage.getItem
    ('authToken')}})
        dispatch({
            type:ADD_SHIPPING,
            payload:res.data
...
export const deleteShipping = (id)=>async dispatch =>{
    ...
    try {
        const res = await Axios.delete(`${process.env.REACT_APP_API_URL}/api/region/${id}`, {headers:{'x-auth-token':localStorage.getItem
    ('authToken')}})
        dispatch({
